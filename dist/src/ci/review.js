...
function resolveModel(modelStr) {
    ...
    const exactModel = getModel(provider, modelId);
    if (exactModel)
        return exactModel;
    const normalizedModelId = modelId.toLowerCase();
    const suffixMatches = getModels(provider).filter((candidate) => {
    ...
    if (suffixMatches.length > 1) {
        throw new Error(`Ambiguous model "${modelStr}". Matches: ${suffixMatches.map((candidate) => `${candidate.provider}/${candidate.id}`).join(", ")}`);
    }
